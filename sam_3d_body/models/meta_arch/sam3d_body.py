...
class SAM3DBody(BaseModel):
    ...
    def run_inference(
        ...
        hand_rot_consistency_thresh: float = 0.6,
        forearm_align_thresh: float = 0.5,
        wrist_soft_limit: float = 1.0,
        wrist_soft_scale: float = 0.5,
    ):
        """
        Run 3DB inference (optionally with hand detector).

        New parameters (defaults chosen conservatively):
            hand_rot_consistency_thresh: radians threshold for left-right wrist rotation consistency
            forearm_align_thresh: radians threshold for hand-forearm axis alignment
            wrist_soft_limit: radians start of soft wrist penalty
            wrist_soft_scale: how strongly to damp extreme wrist angles (0..1)
        ...
        # --- Constraint A: bilateral rotation consistency (left vs right wrists)
        # pred_global_wrist_rotmat: B x 2 x 3 x 3 (left, right)
        lr_angle = rotation_angle_difference(
            pred_global_wrist_rotmat[:, 0], pred_global_wrist_rotmat[:, 1]
        )  # B
        lr_valid = lr_angle < hand_rot_consistency_thresh  # B
        lr_valid_mask = lr_valid.unsqueeze(1).repeat(1, 2)  # B x 2

        # --- Constraint B: hand-forearm axis alignment
        # Forearm axis approximated by the third column of lowarm_joint_rotations (global axis)
        forearm_axis = lowarm_joint_rotations[:, :, :, 2]  # B x 2 x 3
        wrist_axis = pred_global_wrist_rotmat[:, :, :, 2]  # B x 2 x 3
        dot = (forearm_axis * wrist_axis).sum(dim=-1)
        norms = forearm_axis.norm(dim=-1) * wrist_axis.norm(dim=-1) + 1e-8
        forearm_align_angle = torch.acos(torch.clamp(dot / norms, -1.0, 1.0))  # B x 2
        forearm_align_mask = forearm_align_angle < forearm_align_thresh  # B x 2

        # --- Constraint C: soft wrist limit to avoid extreme angles (damp large Euler angles)
        wrist_angle_mag = wrist_xzy.abs().amax(dim=-1)  # B x 2
        soft_over = torch.clamp((wrist_angle_mag - wrist_soft_limit) / (wrist_soft_limit + 1e-8), min=0.0)
        soft_factor = 1.0 - wrist_soft_scale * soft_over
        soft_factor = soft_factor.clamp(min=1.0 - wrist_soft_scale, max=1.0)
        wrist_xzy = wrist_xzy * soft_factor.unsqueeze(-1)

        # Combine with existing criterion: local wrist pose similarity to fused local wrist
        angle_difference = rotation_angle_difference(
            ori_local_wrist_rotmat, fused_local_wrist_rotmat
        )  # B x 2
        angle_difference_valid_mask = angle_difference < thresh_wrist_angle
        # Final validity mask requires all three constraints + existing hand_valid_mask
        valid_angle = (
            angle_difference_valid_mask & lr_valid_mask & forearm_align_mask & hand_valid_mask
        )
