...
class MHRHead(nn.Module):
    ...
    def compute_hand_constraints(
        self,
        joint_global_rots: torch.Tensor,
        joints_3d: torch.Tensor,
        idxs: dict,
        max_angle_rad: float = 1.2,
    ):
        """Compute hand constraint losses.

        Args:
            joint_global_rots: Tensor of shape [B, N, 3, 3]
            joints_3d: Tensor of shape [B, N, 3]
            idxs: dict with keys: 'l_wrist','r_wrist','l_elbow','r_elbow','l_hand','r_hand'
            max_angle_rad: soft limit threshold in radians

        Returns:
            dict with scalar losses: 'loss_bilateral','loss_axis','loss_wrist'
        """
        loss_bilateral = bilateral_hand_rotation_loss(
            joint_global_rots, idxs["l_wrist"], idxs["r_wrist"]
        )

        loss_axis_l = wrist_forearm_axis_loss(
            joints_3d, idxs["l_elbow"], idxs["l_wrist"], idxs["l_hand"]
        )
        loss_axis_r = wrist_forearm_axis_loss(
            joints_3d, idxs["r_elbow"], idxs["r_wrist"], idxs["r_hand"]
        )
        loss_axis = 0.5 * (loss_axis_l + loss_axis_r)

        loss_wrist_l = wrist_angle_soft_limit(
            joint_global_rots, idxs["l_wrist"], max_angle_rad=max_angle_rad
        )
        loss_wrist_r = wrist_angle_soft_limit(
            joint_global_rots, idxs["r_wrist"], max_angle_rad=max_angle_rad
        )
        loss_wrist = 0.5 * (loss_wrist_l + loss_wrist_r)

        return {
            "loss_bilateral": loss_bilateral,
            "loss_axis": loss_axis,
            "loss_wrist": loss_wrist,
        }


# ----------------------- Hand constraint helper functions -----------------------

def rotation_matrix_to_angle(R: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """Convert rotation matrix/matrices to rotation angle in radians.
    ...
    Returns:
        angles: tensor of shape [...] (radians)
    """
    # trace = R00 + R11 + R22
...
def _normalize(vec: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """L2-normalize last dimension of tensor safely."""
    norm = vec.norm(dim=-1, keepdim=True)
    return vec / (norm + eps)


def bilateral_hand_rotation_loss(
    joint_global_rots: torch.Tensor, idx_l_wrist: int, idx_r_wrist: int
) -> torch.Tensor:
    """Penalize large relative rotation between left and right wrists.

    Args:
        joint_global_rots: [B, N, 3, 3]
        idx_l_wrist / idx_r_wrist: indices of wrist joints

    Returns:
        scalar loss (mean angle)
    """
    Rl = joint_global_rots[:, idx_l_wrist]  # [B,3,3]
    Rr = joint_global_rots[:, idx_r_wrist]
    R_rel = torch.matmul(Rl, Rr.transpose(-1, -2))  # [B,3,3]
    angle = rotation_matrix_to_angle(R_rel)  # [B]
    return angle.mean()


def wrist_forearm_axis_loss(
    joints_3d: torch.Tensor, idx_elbow: int, idx_wrist: int, idx_hand: int
) -> torch.Tensor:
    """Encourage alignment between forearm axis and hand axis.

    Args:
        joints_3d: [B, N, 3]
    """
    elbow = joints_3d[:, idx_elbow]
    wrist = joints_3d[:, idx_wrist]
    hand = joints_3d[:, idx_hand]

    forearm_dir = _normalize(wrist - elbow)
    hand_dir = _normalize(hand - wrist)

    cos_sim = (forearm_dir * hand_dir).sum(dim=-1)
    return (1.0 - cos_sim).mean()


def wrist_angle_soft_limit(
    joint_global_rots: torch.Tensor, idx_wrist: int, max_angle_rad: float = 1.2
) -> torch.Tensor:
    """Soft hinge loss that penalizes wrist rotation angles larger than max_angle_rad."""
    R = joint_global_rots[:, idx_wrist]
    angle = rotation_matrix_to_angle(R)  # [B]
    excess = torch.relu(angle - max_angle_rad)
    return excess.mean()
